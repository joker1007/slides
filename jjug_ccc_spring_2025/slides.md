...
- 外部のDBと通信しない、RDBだけでなくRedisなども
- re-partitionの回避
- offset commitタイミングのコントロール
- RocksDBのチューニング
...
# offset commitのコントロール
Kafka Streamsではconsumerのoffset commitは一定間隔で同期リクエストとして実行される。

大抵はat least onceで動作させることになるので、短いスパンでcommitした方がリトライ時の重複は少なくなる一方で、同期リクエストのオーバーヘッドが発生する。

処理自体が羃等であれば、デフォルトの設定(30秒)より多少長くしても問題無い。
...
RocksDBはキーに使用したバイト列の順番でソートされるので、prefixをコントロールしてscanするとセカンダリインデックスの様なことができる。
また、flushタイミングをコントロールすれば、複雑なデータ構造をシリアライズして格納できる。
応用すると以下の様なことが実現できる。
...
- 上記の処理を透過的に行うイテレーターを実装して、Processor APIで利用している
...
# 疎結合の代償
RPCやWeb APIを利用したマイクロサービスと異なり、他のサービスのことを何も知らなくても動作するため疎結合なマイクロサービス群を構築しやすい。

その反面システム全体像の把握が難しいし、オンボーディングが大変。
また、全体としてのシステムパフォーマンスの把握が難しい。

---

# トレーシングの重要性

最近は、OpenTelemetryがこなれてきてサービス間の繋がりをトレースしやすくなった。
システム同士の依存性を可視化できるツールやサービスも増えてきて、一気に動作イメージを把握し易くなった。

もし、新しくこういったアーキテクチャでシステムを構成するなら、最初からトレーシングの活用を念頭に置いておくことはとても重要。

---

# アーキテクチャ転換の所感

こういった難しさはあったものの、Kafka Streamsとストリームアプリケーションの知見の蓄積、イベント駆動アーキテクチャの採用によって一定の成果は出せたと感じている。

チャネルの追加や新しい機能を作る上で、高いスケーラビリティが求められる箇所を上手く隠蔽しつつ、既存のシステムに影響を与えずに新しいコンポーネントを開発できる様になった。
多くのコンポーネントを開発してきて、Reproというサービスとの相性は十分に良い選択だったと評価している。

全体像の把握し辛さも、分散トレーシングによるシステム構成の可視化、各種メトリックダッシュボードの作成により対策が進んでいる。
...
Java関係だとQuarkusなんかも活用しています。
(Quarkus + Kafka Streamsの話もどこかでやりたい)
