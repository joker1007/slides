...
メッセージバスでサービス間を連携する時に大事なことは、サービス同士がお互いの存在すら知る必要が無い、という状態を維持すること。
...
前述した様にノードやディスク破壊に対してはKafka Brokerからのレストアが可能なので、AWSでKafka Streamsを運用する場合は、高速なエフェメラルストレージと相性が良い。例えば、*i4i*シリーズや*r7gd*などの高速なNVMeストレージが装備されているインスタンスだ。
